...
from HelperFunctions import helpers_and_menu as helpers


@pytest.mark.parametrize("value", [None, False, 0, 0.0, "", [], {}, set()])
def test_is_falsy(value) -> None:
    assert helpers._is_falsy(value) is True
    assert helpers._is_truthy(value) is False


@pytest.mark.parametrize("value", [True, 1, -1, "value", [0], {"x": 0}])
def test_is_truthy(value) -> None:
    assert helpers._is_truthy(value) is True
    assert helpers._is_falsy(value) is False


def test_cls_prints_ansi_clear_sequence(capsys) -> None:
    assert helpers._cls() is None
...
def test_generate_dashes_for_menu_numbers(number: int, expected: str) -> None:
    assert helpers._generate_dashes(number) == expected


def test_generate_dashes_for_title() -> None:
    assert helpers._generate_dashes(5, title=True) == "-----"
...
def test_validate_email(email: str, expected: bool) -> None:
    assert helpers._validate_email(email) is expected
...
def test_menu_returns_only_item_without_prompt(monkeypatch, capsys) -> None:
    ...
    assert helpers._menu("Choose", ["only"]) == "only"
...
def test_menu_retries_after_out_of_range_choice(monkeypatch) -> None:
    ...
    clear = Mock()
    monkeypatch.setattr(helpers, "_cls", clear)
    assert helpers._menu(items=["first", "second", "third"]) == "first"
