...
class TrigFunctions:
    ...
    @staticmethod
    def _sin_radians(x: float) -> float:
        """
        Calculate sine from an angle already in radians.
        ...
        for n in range(10):
            sin_x += (
                ((-1) ** n)
                * (x ** (2 * n + 1))
                / exp_all(2 * n + 1, type_=int)
            )

        return sin_x

    @staticmethod
    def _cos_radians(x: float) -> float:
        """
        Calculate cosine from an angle already in radians.
